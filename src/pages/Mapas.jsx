...
function FormatIcon({ format }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-[0.6rem] font-bold uppercase">{format}</span>
    </button>
  )
}
...
function MapCard({ map, index }) {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="p-5">
        <span className="inline-block text-[0.65rem] font-bold uppercase tracking-wider text-primary-700 mb-1.5">
          {map.category}
        </span>
...
function FilterChip({ label, onRemove }) {
  return (
    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-primary-800 text-white rounded-full text-xs font-medium">
      {label}
      <button
        onClick={onRemove}
...
export default function Mapas() {
  ...
  if (filters.year) {
    activeChips.push({ key: 'year', label: filters.year })
  }
  ...
  const clearAllFilters = () => {
    setFilters({ category: '', department: '', format: '', year: '' })
  }
    ...
    <div className="space-y-8">
      ...
      <motion.div {...fadeUp(0)} className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-text-muted leading-relaxed max-w-lg">
            Explore y descargue la cartografía oficial del Chocó
            Biogeográfico. Información científica curada para
            el análisis territorial y la toma de decisiones.
          </p>
        </div>
      ...
      <motion.div {...fadeUp(0.1)} className="bg-white border border-border rounded-xl p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-[0.7rem] font-bold uppercase tracking-widest text-text-muted">
            Filtros Avanzados
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-4">
          ...
          />
          <FilterSelect
            label="Departamento / Subregión"
            options={MAP_DEPARTMENTS}
        ...
        {activeChips.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-border">
            ...
            <button
              ...
              className="text-xs font-medium text-text-muted hover:text-primary-800 ml-auto transition-colors"
            >
      ...
      {/* Second row (duplicate for visual completeness like Figma) */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {SAMPLE_MAPS.map((map, i) => (
      ...
      <motion.div {...fadeUp(0.3)} className="flex flex-col items-center gap-3">
        <div className="flex items-center gap-2">
          <button
            disabled
            className="w-9 h-9 rounded-lg border border-border bg-white text-text-muted flex items-center justify-center opacity-50"
          >
          ...
          {[1, 2, 3].map((n) => (
            <button
              key={n}
              className={`w-9 h-9 rounded-lg border text-sm font-medium flex items-center justify-center transition-colors ${
                n === 1
                  ? 'bg-primary-800 border-primary-800 text-white'
                  : 'border-border bg-white text-text-light hover:bg-primary-800 hover:border-primary-800 hover:text-white'
              }`}
            >
              {n}
            </button>
          ...
          <button className="w-9 h-9 rounded-lg border border-border bg-white text-text-light text-sm font-medium flex items-center justify-center hover:bg-primary-800 hover:border-primary-800 hover:text-white transition-colors">
            12
          </button>
        ...
        <span className="text-xs text-text-muted">
          Mostrando 12 de 1,248 resultados
        </span>
      </motion.div>
    </div>
  )

...
// Formulario de solicitud de acceso — vive dentro de un panel centrado
// (CenteredAuthPanel), no en una página aparte. Los enlaces "ya tengo
// cuenta" cierran este panel y navegan a /login, que reenvía a "/" abriendo
// el panel de login (ver src/pages/auth/Login.tsx).

export default function SolicitarAccesoForm({ onClose }: { onClose: () => void }) {
  ...
  const backToLogin = () => { onClose(); navigate('/login') }
    ...
    <AnimatePresence mode="wait">
      ...
      {sent ? (
        ...
        >
          ...
          <h3 className="font-display text-xl font-bold text-text mb-2">¡Verifica tu correo!</h3>
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-5 py-3 bg-primary-800 text-white rounded-xl text-sm font-semibold hover:bg-primary-700 transition-colors w-full"
          >
        ...
        >
          <p className="text-sm text-text-muted mb-5">Complete el formulario. Recibirá un correo para verificar su cuenta.</p>
            ...
            <div>
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-xs text-text-muted leading-relaxed">
                  Acepto los{' '}
                  <Link to="/terminos" onClick={onClose} className="font-semibold text-primary-800 hover:underline no-underline">

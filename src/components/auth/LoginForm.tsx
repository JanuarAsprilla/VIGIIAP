...
/**
 * Formulario de acceso compartido — usado tanto dentro del panel anclado del
 * TopBar (LoginPanel) como en cualquier contexto de página completa. Toda la
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (modo === 'visitante') {
        ...
        onClose?.()
        navigate(safeFrom === '/admin' ? '/' : safeFrom, { replace: true })
      } else {
        ...
        const safeTo  = safeFrom === '/admin' && !isAdmin ? '/' : safeFrom
        onClose?.()
        ...
        {modo === 'visitante' && (
          ...
          >
            ...
            <p className="text-[0.65rem] text-text-muted/70 text-center leading-relaxed">
              ...
              <Link to="/terminos" onClick={onClose} className="hover:underline">
